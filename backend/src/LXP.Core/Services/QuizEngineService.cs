...
public class QuizEngineService(IQuizEngineRepository quizEngineRepository) : IQuizEngineService
{
    private readonly IQuizEngineRepository _quizEngineRepository = quizEngineRepository;

    public async Task<ViewQuizDetailsViewModel> GetQuizByIdAsync(Guid quizId) =>
        await this._quizEngineRepository.GetQuizByIdAsync(quizId);

    public async Task<IEnumerable<QuizEngineQuestionViewModel>> GetQuestionsForQuizAsync(
        Guid quizId
    ) => await this._quizEngineRepository.GetQuestionsForQuizAsync(quizId);

    public async Task<ViewQuizDetailsViewModel> GetQuizDetailsByTopicIdAsync(Guid topicId) =>
        await this._quizEngineRepository.GetQuizDetailsByTopicIdAsync(topicId);
    ...
    public async Task<Guid> StartQuizAttemptAsync(Guid learnerId, Guid quizId)
    {
        ...
        var isAllowedToAttempt = await this._quizEngineRepository.IsAllowedToAttemptQuizAsync(
            learnerId,
            quizId
        );
        if (!isAllowedToAttempt)
        {
            var existingAttempts = await this._quizEngineRepository.GetLearnerAttemptsForQuizAsync(
                learnerId,
                quizId
            );
        ...
        var attempt =
            await this._quizEngineRepository.CreateLearnerAttemptAsync(learnerId, quizId, startTime)
            ?? throw new InvalidOperationException(
                "You have exceeded the maximum number of attempts for this quiz."
            );
    ...
    {
        var attempt =
            await this._quizEngineRepository.GetLearnerAttemptByIdAsync(
        ...
        await this._quizEngineRepository.ClearLearnerAnswersAsync(
            answerSubmissionModel.LearnerAttemptId,
            answerSubmissionModel.QuizQuestionId
        );

        var availableOptions = await this._quizEngineRepository.GetQuestionOptionsAsync(
            answerSubmissionModel.QuizQuestionId
        );
        var availableOptionsIgnoreCase = availableOptions
            .Select(o => o.ToLower(System.Globalization.CultureInfo.CurrentCulture))
            .ToList();
        var questionType = await this._quizEngineRepository.GetQuestionTypeByIdAsync(
            answerSubmissionModel.QuizQuestionId
        );
        ...
        switch (questionType)
        {
            ...
            case "T/F":
                ...
                break;
            case "MSQ":
                if (answerSubmissionModel.SelectedOptions.Count is < 2 or > 3)
                {
                    throw new InvalidOperationException(
        ...
        foreach (var selectedOption in answerSubmissionModel.SelectedOptions)
        {
            ...
            var optionId = await this._quizEngineRepository.GetOptionIdByTextAsync(
                answerSubmissionModel.QuizQuestionId,
                optionTextLower
            );
            await this._quizEngineRepository.CreateLearnerAnswerAsync(
                answerSubmissionModel.LearnerAttemptId,
                answerSubmissionModel.QuizQuestionId,
                optionId
            );
    ...
    public async Task<LearnerPassStatusViewModel> CheckLearnerPassStatusAsync(Guid learnerAttemptId)
    {
        var attempt =
            await this._quizEngineRepository.GetLearnerAttemptByIdAsync(learnerAttemptId)
            ?? throw new KeyNotFoundException(
                $"Learner attempt with ID {learnerAttemptId} not found."
            );

        var quiz =
            await this._quizEngineRepository.GetQuizByIdAsync(attempt.QuizId)
    ...
    {
        var attempt =
            await this._quizEngineRepository.GetLearnerAttemptByIdAsync(attemptId)
            ?? throw new KeyNotFoundException($"Learner attempt with ID {attemptId} not found.");
        var quiz =
            await this._quizEngineRepository.GetQuizByIdAsync(attempt.QuizId)
            ?? throw new KeyNotFoundException($"Quiz with ID {attempt.QuizId} not found.");
        var totalQuestions = (
            await this._quizEngineRepository.GetQuestionsForQuizAsync(quiz.QuizId)
        ).Count();
        ...
        {
            var isAnswerCorrect = await this._quizEngineRepository.IsQuestionOptionCorrectAsync(
                answer.QuizQuestionId,
                answer.QuestionOptionId
            );
            var questionScore = await this.CalculateQuestionScore(
                answer.QuizQuestionId,
                isAnswerCorrect,
                ...
                new AnswerSubmissionModel
                {
                    ...
                    SelectedOptions =
                    [
                        await this._quizEngineRepository.GetOptionTextByIdAsync(
                            answer.QuestionOptionId
                        )
                    ]
                }
        ...
        await this._quizEngineRepository.UpdateLearnerAttemptAsync(attempt);
    }
    ...
    public async Task<Guid> RetakeQuizAsync(Guid learnerId, Guid quizId)
    {
        ...
        var startTime = DateTime.Now;
        var attempt = await this._quizEngineRepository.CreateLearnerAttemptAsync(
            learnerId,
            quizId,
            startTime
        );
    ...
    )
    {
        ...
        switch (questionType)
        {
            ...
            case "MSQ":
                var correctOptions =
                    await this._quizEngineRepository.GetCorrectOptionsForQuestionAsync(
                        quizQuestionId
                    );
                ...
                {
                    var partialMark =
                        individualQuestionMarks / correctOptionCount * correctlySelectedOptions;
                    return partialMark; // Partial marks for partially correct answer and no incorrect options
                }
                else
                {
                    return individualQuestionMarks
                        * (correctlySelectedOptions - incorrectlySelectedOptions)
                        / correctOptionCount; // Adjusted for incorrect options
                }
    ...
    public async Task<LearnerQuizAttemptViewModel> GetLearnerQuizAttemptAsync(Guid attemptId) =>
        await this._quizEngineRepository.GetLearnerQuizAttemptAsync(attemptId);

    public async Task<LearnerQuizAttemptResultViewModel> GetLearnerQuizAttemptResultAsync(
        Guid attemptId
    ) => await this._quizEngineRepository.GetLearnerQuizAttemptResultAsync(attemptId);

    public async Task<LearnerQuizStatusViewModel> GetLearnerQuizStatusAsync(
        Guid learnerId,
        Guid quizId
    ) => await this._quizEngineRepository.GetLearnerQuizStatusAsync(learnerId, quizId);
}
