...
public class TopicFeedbackController(ITopicFeedbackService service) : BaseController
{
    private readonly ITopicFeedbackService _service = service;
    ...
    {
        var question = this._service.GetFeedbackQuestionById(topicFeedbackQuestionId);
        if (question == null)
        {
    ...
    {
        var existingQuestion = this._service.GetFeedbackQuestionById(topicFeedbackQuestionId);
    ...
    public IActionResult DeleteFeedbackQuestion(Guid topicFeedbackQuestionId)
    {
        ...
        var result = this._service.DeleteFeedbackQuestion(topicFeedbackQuestionId);
    ...
    {
        var questions = this._service.GetFeedbackQuestionsByTopicId(topicId);
        if (questions == null || questions.Count == 0)
        {
            return this.NotFound(
                this.CreateFailureResponse("No questions found for the given topic", 404)
            );
        }
