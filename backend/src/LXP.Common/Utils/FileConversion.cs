...
public static class FileConversion
{
    ...
    )
    {
        ...
        var correctedFilePath = FilePath
            ...
            .Replace("/", "\\");
        var fullFilePath = Path.Combine(webRootPath, correctedFilePath);
        ...
        if (materialType == "PPT")
        {
            ...
            var pdfFilePath = Path.ChangeExtension(fullFilePath, ".pdf");

            // Save the presentation as a PDF
            presentation.SaveToFile(pdfFilePath, Spire.Presentation.FileFormat.PDF);
            var relativePptPdfPath = pdfFilePath.Replace(webRootPath, "").Replace("\\", "/");
        ...
        else if (materialType == "TEXT")
        {
            ...
            // Define the output PDF file path
            var docFilePath = Path.ChangeExtension(fullFilePath, ".pdf");
            // Save the document as a PDF
            document.SaveToFile(docFilePath, Spire.Doc.FileFormat.PDF);
            var relativeDocPdfPath = docFilePath.Replace(webRootPath, "").Replace("\\", "/");
